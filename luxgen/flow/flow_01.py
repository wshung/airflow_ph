...
query_string = """
SELECT cmain_lxg_people.yl_id,
sso_t.sso_id,
cmain_lxg_people.pii_token,
cmain_lxg_people.member_type,
cmain_lxg_people.opp_status,
cmain_lxg_people.is_car_owner,
cmain_lxg_people.is_car_driver,
cmain_lxg_people.line_id,
cmain_lxg_people.registration_time,
cmain_lxg_people.registration_channel,
cmain_lxg_people.birthday,
cmain_lxg_people.gender,
cmain_lxg_people.residence_city,
cmain_lxg_people.residence_district,
cmain_lxg_people.first_opp_time,
cmain_lxg_people.last_opp_time,
cmain_lxg_people.first_test_drive_time,
cmain_lxg_people.last_test_drive_time,
cmain_lxg_people.first_buy_time,
cmain_lxg_people.last_buy_time,
cmain_lxg_people.first_deliver_time,
cmain_lxg_people.last_deliver_time
FROM (info.cmain_lxg_people
left join (select yl_id, sso_id from info.cmain_ylg_people cyp where is_sso_member = 1) sso_t
on sso_t.yl_id = cmain_lxg_people.yl_id)
;
"""
...
lxg_ower_splant_distance = pd.read_excel('.\lxg_ower_splant_distance.xlsx', dtype={'master_mobile': str})

# n7預購第一波名單-純電代言人

lxg_n7_order_1st = pd.read_excel('./lxg_n7_order_1st_0310.xlsx', dtype={'手機號碼': str})

# n7預購第二波名單-純電夥伴

lxg_n7_order_2nd = pd.read_excel('./lxg_n7_order_2nd_0310.xlsx', dtype={'手機號碼': str})

# n7預購退購名單-latest

lxg_n7_order_cancel_new = pd.read_excel('./lxg_n7_order_cancel.xlsx', dtype={'手機號碼': str})

# n7-NFT盲盒領取開啟
lxg_n7_nft_list = pd.read_excel('./lxg_n7_nft_list_202401160014.xlsx', dtype={'uuid': str})
...
from utils.helpers import get_age_range, get_years, get_oppstatus, get_regist, get_gender, get_caryears, get_YN, get_YNO, change_language , delete_old_tag, write_into_tag_table
...
cmain_lxg_people_membertag['registration_time'] = cmain_lxg_people['registration_time']
...
cmain_lxg_people_membertag['lxg_first_opp_time'] = cmain_lxg_people['first_opp_time']
cmain_lxg_people_membertag['lxg_last_opp_time'] = cmain_lxg_people['last_opp_time']
cmain_lxg_people_membertag['lxg_first_test_drive_time'] = cmain_lxg_people['first_test_drive_time']
cmain_lxg_people_membertag['lxg_last_test_drive_time'] = cmain_lxg_people['last_test_drive_time']
cmain_lxg_people_membertag['lxg_first_buy_time'] = cmain_lxg_people['first_buy_time']
cmain_lxg_people_membertag['lxg_last_buy_time'] = cmain_lxg_people['last_buy_time']
cmain_lxg_people_membertag['lxg_first_deliver_time'] = cmain_lxg_people['first_deliver_time']
cmain_lxg_people_membertag['lxg_last_deliver_time'] = cmain_lxg_people['last_deliver_time']
...
# 是否為過保固車
lxg_nv_vehicle['is_warrantyexpiry'] = pd.to_datetime(lxg_nv_vehicle['warrantyexpiry']).dt.date.transform(lambda x: 1 if  x <= today else 0)
cmain_lxg_people_membertag = cmain_lxg_people_membertag.join(lxg_nv_vehicle.loc[:,["ownermobile_09", "is_warrantyexpiry"]].set_index('ownermobile_09'), on='mobile')
cmain_lxg_people_membertag['is_warrantyexpiry'] = cmain_lxg_people_membertag['is_warrantyexpiry'].transform(get_YN)


cmain_lxg_people_membertag['is_warrantyexpiry'] 


# 是否為過保險車
lxg_nv_vehicle['is_insuranceexpiry'] = pd.to_datetime(lxg_nv_vehicle['insuranceexpiry']).dt.date.transform(lambda x: 1 if  x <= today else 0)
cmain_lxg_people_membertag = cmain_lxg_people_membertag.join(lxg_nv_vehicle.loc[:,["ownermobile_09", "is_insuranceexpiry"]].set_index('ownermobile_09'), on='mobile')
...
# # 車齡(最近一部車車齡)
lxg_nv_vehicle['carage'] = (today - pd.to_datetime(lxg_nv_vehicle['deliverydate']).dt.date).transform(get_caryears)
lxg_nv_vehicle_tem = lxg_nv_vehicle[['ownermobile_09','carage']]
...
# 是否還有紅利點數
lxg_nv_vehicle['is_bonus'] = lxg_nv_vehicle['bonus'].transform(lambda x: 1 if  x >= 0 else 0)
cmain_lxg_people_membertag = cmain_lxg_people_membertag.join(lxg_nv_vehicle.loc[:,["ownermobile_09", "is_bonus"]].set_index('ownermobile_09'), on='mobile')
cmain_lxg_people_membertag['is_bonus'] = cmain_lxg_people_membertag['is_bonus'].transform(get_YN)


# # 紅利點數是否到期 
lxg_nv_vehicle['is_bonusexpiry'] = lxg_nv_vehicle['bonusexpiry'].transform(lambda x: 1 if ((x is not None) and (x <= today)) else  0 if ((x is not None) and (x > today)) else  None)
cmain_lxg_people_membertag2 = cmain_lxg_people_membertag2.join(lxg_nv_vehicle.loc[:,["ownermobile_09", "is_bonusexpiry"]].set_index('ownermobile_09'), on='mobile')
...
Q1 = cmain_lxg_people_ctb_tsell['tcost_1y'].quantile(0.25)
Q3 = cmain_lxg_people_ctb_tsell['tcost_1y'].quantile(0.75)
cmain_lxg_people_ctb_tsell['contribution'] = cmain_lxg_people_ctb_tsell['tcost_1y'].transform(get_lxg_customer_contribution)
...
Q1 = cmain_lxg_people_ctb_loyalty['tcost_1y'].quantile(0.25)
Q3 = cmain_lxg_people_ctb_loyalty['tcost_1y'].quantile(0.75)
fQ1 = cmain_lxg_people_ctb_loyalty['cs_tfreq_1y'].quantile(0.25)
fQ3 = cmain_lxg_people_ctb_loyalty['cs_tfreq_1y'].quantile(0.75)

cmain_lxg_people_ctb_loyalty['loyalty'] = cmain_lxg_people_ctb_loyalty.apply(lambda x: get_lxg_customer_loyalty(x['tcost_1y'], x['cs_tfreq_1y']), axis=1)
...
lxg_nv_testdrive_oneyear.columns
...
lxg_nv_testdrive_latest_td['day_to_lattdrive'].dtypes
