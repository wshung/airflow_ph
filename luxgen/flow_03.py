...
lxg_nv_opp_cst['day_diff'] =  (today - pd.to_datetime(lxg_nv_opp_cst['crt_date'], errors='coerce').dt.date)
# lxg_nv_opp_cst['day_diff'] =  lxg_nv_opp_cst['day_diff'].dt.days
lxg_nv_opp_cst['day_diff'] = lxg_nv_opp_cst['day_diff'].apply(lambda x: x.days)
# 篩選1年內的有望客
...
lxg_nv_testdrive['day_diff'] =  today - pd.to_datetime(lxg_nv_testdrive['testdrivedate'], errors='coerce').dt.date
# lxg_nv_testdrive['day_diff'] =  lxg_nv_testdrive['day_diff'].dt.days
lxg_nv_testdrive['day_diff'] =  lxg_nv_testdrive['day_diff'].apply(lambda x: x.days)
...
# 計算成為試駕日期和資料更新日期的差幾天(days)
lxg_nv_crm['day_diff'] =  (today - pd.to_datetime(lxg_nv_crm['opendate'], errors='coerce').dt.date)
# lxg_nv_crm['day_diff'] =  lxg_nv_crm['day_diff'].dt.days
lxg_nv_crm['day_diff'] =  lxg_nv_crm['day_diff'].apply(lambda x: x.days)
...
print(people_offline_df_latest.head(100))
print(people_offline_df_3m.head(100))
print(people_offline_df_oneyear.head(100))
...
tag_table_test03 = tag_table.head(10000)

print(len(tag_table_test03))

print(tag_table_test03)


for i in range(0,len(tag_table_test03),10000):
    write_into_tag_table(df = tag_table_test03.iloc[i:i+10000], table_name = 'lxg_tag', schema_name = 'dt_tag', method = 'keep')
